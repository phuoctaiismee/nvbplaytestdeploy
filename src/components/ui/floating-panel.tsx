...
export function useFloatingPanelLogic() {
  const uniqueId = useId();
  const [isOpen, setIsOpen] = useState(false);
  const [note, setNote] = useState("");
  const [triggerRect, setTriggerRect] = useState<DOMRect | null>(null);
  const [title, setTitle] = useState("");
  const openFloatingPanel = (rectOrEvent?: DOMRect | MouseEvent | null) => {
    if (rectOrEvent instanceof MouseEvent) {
      const target = rectOrEvent.currentTarget as HTMLElement;
      setTriggerRect(target.getBoundingClientRect());
    } else if (rectOrEvent) {
      setTriggerRect(rectOrEvent);
    }
    setIsOpen(true);
  };
...
export function FloatingPanelRoot({
  ...
  // Xác định xem component đang ở controlled hay uncontrolled mode
  const isControlled = controlledOpen !== undefined;
  const isOpen = isControlled ? controlledOpen : uncontrolledOpen;
  const floatingPanelLogic = useFloatingPanelLogic();

  // Override isOpen và các handlers
  const contextValue = {
    ...floatingPanelLogic,
    isOpen,
    openFloatingPanel: (rect?: DOMRect) => {
      if (isControlled) {
        onOpenChange?.(true);
      } else {
        setUncontrolledOpen(true);
      }
      floatingPanelLogic.openFloatingPanel(rect);
    },
    closeFloatingPanel: () => {
      if (isControlled) {
        onOpenChange?.(false);
      ...
      floatingPanelLogic.closeFloatingPanel();
    },
  };
  return (
    // @ts-ignore
