...
>((props, ref) => {
  ...
  const toggleUpdateState = React.useCallback(
    () => setUpdateState((previousState) => !previousState),
    []
  );
    ...
    <div ref={ref} className={`flex justify-center ${props.className}`}>
      {Array.from({ length: numberOfSlides }, (_, index) => (
        <Button
          key={index}
          variant="ghost"
          className={`mx-1 h-1 w-6 rounded-full p-0 ${
            index === currentSlide
              ? `${props.dotActiveClassName}` || "scale-125 transform bg-white"
              : `${props.dotClassName}` || "bg-white/50 hover:bg-neutral-300"
          }`}
