...
// ********************** BLOG COMMENT **********************
/**
 * The function `getAnonymousId` retrieves the value of the "anonymous_comment_id" cookie.
 * @returns The `getAnonymousId` function is returning the value of the cookie named
 * "anonymous_comment_id" as a string, or `undefined` if the cookie does not exist.
 */
export const getAnonymousId = (): string | undefined => {
  return Cookies.get("anonymous_comment_id");
};

/**
 * The function `setAnonymousId` sets an anonymous comment ID in a cookie with a one-year expiration.
 * @param {string} anonymousId - The `anonymousId` parameter is a string that represents the unique
 * identifier for an anonymous user. This identifier is used to track and associate comments or actions
 * with the specific anonymous user.
 */
export const setAnonymousId = (anonymousId: string): void => {
  Cookies.set("anonymous_comment_id", anonymousId, { expires: 365 });
};

/**
 * The function `removeAnonymousId` removes the "anonymous_comment_id" cookie.
 */
export const removeAnonymousId = (): void => {
  Cookies.remove("anonymous_comment_id");
};
